 ...
 function checkForm(form)
  {
    ...
    result=confirm("Promote students : " + itemsChecked );
	if(result)
	{
		return true;
	}
...
mysql_select_db("attendance");
	...
	{
		$query=mysql_query("SELECT gr_no, roll_no,division,year,batch FROM $stream WHERE trim='$trim' AND year<='$cur_year' AND year>='$valid_year' ORDER BY roll_no") or die(mysql_error());
	}
...
<form name="myForm" method="post" action="student_propg3.php" onsubmit="return checkForm(this)" >
...
<input name="submit1" type="submit" id="submit1" value="Promote"/>
</form>
<?
}
	else
